...
// ESG 도표(테이블) 저장용 - charts E/S/G 탭에서 저장한 테이블
export interface SavedEsgTableColumn {
  key: string;
  label: string;
  align?: 'left' | 'center' | 'right';
}
export interface SavedEsgTableRow {
  id: string;
  cells: Record<string, string>;
}
export interface SavedEsgTable {
  id: string;
  presetId: string;
  title: string;
  note?: string;
  columns: SavedEsgTableColumn[];
  rows: SavedEsgTableRow[];
  createdAt: Date;
}
...
export interface ReportStore {
  ...
  // Actions - ESG 도표(테이블) - charts E/S/G 탭
  addEsgTable: (table: Omit<SavedEsgTable, 'id' | 'createdAt'>) => void;
  removeEsgTable: (id: string) => void;
...
export const useReportStore = create<ReportStore>()(
  persist(
    (set, get) => ({
      ...
      // ESG 도표(테이블) 추가
      addEsgTable: (table) => {
        const newTable: SavedEsgTable = {
          ...table,
          id: `esg-table-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
          createdAt: new Date(),
        };
        set({ esgTables: [...get().esgTables, newTable] });
        get().updateReportMetadata({ lastUpdated: new Date() });
      },

      // ESG 도표(테이블) 제거
      removeEsgTable: (id) => {
        set({ esgTables: get().esgTables.filter((t) => t.id !== id) });
        get().updateReportMetadata({ lastUpdated: new Date() });
      },
    ...
    }),
    {
      ...
      partialize: (state: ReportStore) => ({
        ...
        esgTables: state.esgTables.map((t) => ({
          ...t,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          createdAt: t.createdAt.toISOString() as any,
        })),
        reportMetadata: {
      ...
      onRehydrateStorage: () => (state: any) => {
        if (state) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          if (state.esgTables?.length) {
            state.esgTables = state.esgTables.map((t: any) => ({
              ...t,
