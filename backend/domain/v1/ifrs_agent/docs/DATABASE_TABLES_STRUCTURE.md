...
**주요 필드** (마이그레이션: `035_ghg_activity_data` 기본 생성, `036_ghg_act_sds_cols`에서 SDS·DC·Scope3 등 확장):
...
CREATE TABLE ghg_activity_data (
  ...
  -- 탭 구분 (CHECK 제약)
  -- 기본 6종 + 확장: water_usage | pure_water | air_emissions | renewable_energy | scope3_activity
  ...
  period_quarter INTEGER,  -- 분기 단위 원본(대기·폐기물·Scope3 등)

  -- 계보·멱등 적재 (EMS record_id, GHG-S12 id 등)
  ...
  usage_unit TEXT,
  renewable_ratio DECIMAL(5, 2),
  renewable_kwh DECIMAL(18, 4),
  non_renewable_kwh DECIMAL(18, 4),
  pue_monthly DECIMAL(10, 4),
  it_load_kw DECIMAL(18, 4),
  cooling_power_kwh DECIMAL(18, 4),
  grid_emission_factor_market DECIMAL(18, 6),
  grid_emission_factor_location DECIMAL(18, 6),
  calculated_ghg_market_tco2e DECIMAL(18, 4),
  calculated_ghg_location_tco2e DECIMAL(18, 4),
  meter_id TEXT,

  -- 연료·차량 (tab_type = 'fuel_vehicle') + GHG Scope1/2 요약 행 매핑 시
  fuel_category TEXT,
  fuel_type TEXT,
  consumption_amount DECIMAL(18, 4),
  fuel_unit TEXT,
  purchase_amount DECIMAL(18, 4),
  ghg_accounting_basis TEXT,  -- '시장기반' | '위치기반' 등
  fuel_code TEXT,
  emission_factor_id TEXT,
  ghg_co2_tco2e DECIMAL(18, 4),
  ghg_ch4_tco2e DECIMAL(18, 4),
  ghg_n2o_tco2e DECIMAL(18, 4),
  ghg_hfcs_tco2e DECIMAL(18, 4),
  ghg_total_tco2e DECIMAL(18, 4),
  ...
  waste_type TEXT,
  waste_name TEXT,
  generation_amount DECIMAL(18, 4),
  disposal_method TEXT,
  incineration_amount DECIMAL(18, 4),
  recycling_amount DECIMAL(18, 4),
  recycling_rate_pct DECIMAL(5, 2),
  landfill_rate_pct DECIMAL(5, 2),
  incineration_rate_pct DECIMAL(5, 2),
  hazardous_waste_yn TEXT,
  treatment_contractor TEXT,
  ...
  weight_ton DECIMAL(18, 4),
  person_trips INTEGER,
  ...
  ghg_reported_yn TEXT,

  -- 용수·배출 (tab_type = 'water_usage') — ENV_WATER_DETAIL 등
  water_source TEXT,
  water_intake_ton DECIMAL(18, 4),
  water_discharge_ton DECIMAL(18, 4),
  water_reuse_ton DECIMAL(18, 4),
  water_consumption_ton DECIMAL(18, 4),
  water_reuse_rate_pct DECIMAL(5, 2),
  water_stress_area_yn TEXT,
  cooling_tower_makeup_ton DECIMAL(18, 4),
  water_blowdown_ton DECIMAL(18, 4),
  water_evaporation_site_ton DECIMAL(18, 4),
  wue_l_kwh DECIMAL(18, 6),
  water_discharge_destination TEXT,
  discharge_quality_compliant_yn TEXT,

  -- 순수·공정수 (tab_type = 'pure_water') — EMS_PUREWATER_USAGE 등
  site_type_label TEXT,
  usage_purpose TEXT,
  intake_source TEXT,
  raw_water_m3 DECIMAL(18, 4),
  pure_water_m3 DECIMAL(18, 4),
  purewater_conversion_ratio DECIMAL(10, 6),
  return_water_m3 DECIMAL(18, 4),
  process_evaporation_m3 DECIMAL(18, 4),
  conductivity_us_cm DECIMAL(18, 6),
  resistivity_mohm_cm DECIMAL(18, 4),
  water_usage_cost_krw DECIMAL(18, 2),

  -- 대기 배출 (tab_type = 'air_emissions') — EHS_AIR_EMISSION
  emission_source_description TEXT,
  air_source_fuel_type TEXT,
  operation_hours DECIMAL(18, 4),
  nox_kg DECIMAL(18, 4),
  sox_kg DECIMAL(18, 4),
  dust_kg DECIMAL(18, 4),
  co_kg DECIMAL(18, 4),
  voc_kg DECIMAL(18, 4),
  nox_conc_ppm DECIMAL(18, 4),
  sox_conc_ppm DECIMAL(18, 4),
  dust_conc_mg_m3 DECIMAL(18, 4),
  regulatory_limit_nox DECIMAL(18, 4),
  air_compliance_status TEXT,
  air_measurement_method TEXT,
  measurement_agency TEXT,

  -- 재생에너지 (tab_type = 'renewable_energy') — EMS_RENEWABLE_ENERGY
  re_type TEXT,
  re_source TEXT,
  re_generation_kwh DECIMAL(18, 4),
  re_consumption_kwh DECIMAL(18, 4),
  certificate_type TEXT,
  certificate_volume_rec DECIMAL(18, 4),
  certificate_cost_krw DECIMAL(18, 2),
  re_co2_reduction_tco2e DECIMAL(18, 4),
  grid_displacement_factor DECIMAL(18, 6),

  -- Scope3 활동·산정 (tab_type = 'scope3_activity') — GHG_SCOPE3_DETAIL
  scope3_category TEXT,
  scope3_category_name TEXT,
  scope3_subcategory TEXT,
  scope3_calculation_method TEXT,
  scope3_activity_amount DECIMAL(18, 4),
  scope3_activity_unit TEXT,
  scope3_emission_factor DECIMAL(18, 6),
  scope3_ef_unit TEXT,
  scope3_ef_source TEXT,
  scope3_ghg_emission_tco2e DECIMAL(18, 4),
  scope3_boundary TEXT,
  scope3_source_file TEXT,
  scope3_notes TEXT,

  -- 데이터 품질 및 출처
  data_quality TEXT,
  source_system TEXT,  -- 'EMS' | 'ERP' | 'EHS' | 'GHG' | 'SRM' | 'HR' | 'PLM' | 'MDG' | 'manual' 등
  synced_at TIMESTAMPTZ,
  updated_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  CONSTRAINT chk_ghg_activity_data_tab_type CHECK (tab_type IN (
    'power_heat_steam', 'fuel_vehicle', 'refrigerant', 'waste', 'logistics_travel', 'raw_materials',
    'water_usage', 'pure_water', 'air_emissions', 'renewable_energy', 'scope3_activity'
  )),

  INDEX idx_ghg_activity_company (company_id, period_year, period_month),
  INDEX idx_ghg_activity_tab (company_id, tab_type),
  -- source_record_id 가 있는 행만 (PostgreSQL 부분 인덱스)
  INDEX idx_ghg_activity_source_rec (company_id, source_system, source_record_id)
    WHERE source_record_id IS NOT NULL
);
```

**소스 보고값 vs 플랫폼 산정값**: 활동량·단위·사업장·기간 등은 전형적인 **입력(재료)** 이다. 한편 `calculated_ghg_*_tco2e`, `ghg_*_tco2e`, `scope3_ghg_emission_tco2e`, `re_co2_reduction_tco2e` 등 **배출·감축에 가까운 숫자 컬럼**은 SDS/EMS/GHG 등 **원천이 이미 산출해 넣은 값을 보존**하기 위한 희소 필드로 쓴다(대사·감사·소스 추적). 플랫폼이 배출계수를 적용해 수행한 **공식 집계 결과**는 **`ghg_emission_results`**에 저장하고, 행 단위 산정 근거·스냅샷은 **`ghg_calculation_evidence`**에 둔다(아래 `ghg_calculation_evidence` 절 데이터 흐름 참고).

**데이터 소스**:
- EMS: 전력·열·스팀, DC PUE·IT부하, 폐기물(분기), 용수·WUE, 순수/공정수, 재생에너지, Scope1·2·3 산정 원본
- ERP: 연료·차량
- EHS: 냉매, 대기 배출(분기)
- GHG(또는 EMS 내 GHG 모듈): Scope1/2 요약, Scope3 Cat.별 활동데이터
- SRM: 물류, 원료
- HR: 출장·통근
- PLM: 제품
...
- 예: 2024년 1월 데이터가 있다면
  ...
  - `tab_type = 'water_usage'` → 사업장·기간별 용수·배출·재이용·WUE 등
  - `tab_type = 'pure_water'` → 순수/공정수(냉각탑 보충 등) 월별
  - `tab_type = 'air_emissions'` → 배출원·분기별 대기오염물질
  - `tab_type = 'renewable_energy'` → 재생 설비·인증서별
  - `tab_type = 'scope3_activity'` → Scope3 Cat.별 활동량·배출량 한 줄
...
**행 단위(구현 기준)**: 물리적으로는 **와이드 테이블 하나**이지만, 논리적 식별자는 **`(company_id, data_type, period_year)`** 입니다. 동일 회사·동일 연도에 대해 `data_type`마다 **최대 1행**(총 최대 4행: `workforce`, `safety`, `supply_chain`, `community`). 각 행에서는 해당 유형에 맞는 지표 컬럼만 채우고 나머지는 NULL로 둡니다.

**제약·인덱스(Alembic 019·034)**:
- `company_id` → `companies.id` **FOREIGN KEY**
- `chk_social_data_type`: `data_type IN ('workforce', 'safety', 'supply_chain', 'community')`
- **UNIQUE** `uq_social_company_type_year` (`company_id`, `data_type`, `period_year`)
- `idx_social_company` (`company_id`, `period_year`), `idx_social_status` (`company_id`, `status`)
...
CREATE TABLE social_data (
  ...
  CONSTRAINT chk_social_data_type CHECK (
    data_type IN ('workforce', 'safety', 'supply_chain', 'community')
  )
);
CREATE UNIQUE INDEX uq_social_company_type_year
  ON social_data (company_id, data_type, period_year);
CREATE INDEX idx_social_company ON social_data (company_id, period_year);
CREATE INDEX idx_social_status ON social_data (company_id, status);
```

**데이터 소스**:
- HR 시스템 (임직원, 일부 안전·공헌 지표)
- SRM 시스템 (협력회사)
- EHS 등 (안전 지표, HR과 함께 `safety` 행 집계에 사용 가능)
- 수동 입력 (사회공헌)

**스테이징 → social_data 변환**: 스테이징(`staging_hr_data`, `staging_srm_data`, `staging_ehs_data` 등)의 `raw_data.items`를 연도·도메인 규칙으로 **집계한 뒤**, `data_type`별로 **`(company_id, data_type, period_year)` 자연키에 맞춰 upsert** 합니다. 예: HR → `workforce`(및 필요 시 `community`), SRM → `supply_chain`, HR+EHS 합산 → `safety`. 한 회사·연도에 여러 소스가 있어도 **한 `data_type`당 한 행**이 되도록 소스별 지표를 머지·집계한 다음 저장합니다. 백엔드 참고: `SocialDataBuildService`, `social_staging_extract`. 상세 패턴은 [6.3 권장 개선 구조](#63-권장-개선-구조) 내 "스테이징 → social_data 변환" 참고.
...
1. **사용자 정보**: `users` (온프레미스용 — `name`, `permission`, 초기 비밀번호·최초 로그인 관리 포함)
2. **회사 정보**: `companies` (회사 계정 `company_login_id`·`company_password_hash`, 그룹 구분 `group_entity_type`·`parent_company_id`, 마감일, 최종 승인 정보 포함)
...
**스테이징 → social_data 변환: CTE + 집계 후 `data_type`별 upsert**

`social_data`는 **회사·연도·`data_type`당 1행**(자연키 유니크). 같은 회사·같은 연도라도 `workforce` / `safety` / `supply_chain` / `community`는 **서로 다른 행**으로 저장됩니다.

- **소스별 역할**: HR 스테이징은 주로 `workforce`(및 `community`) 지표, SRM은 `supply_chain`, EHS(및 HR의 안전 관련 항목)는 `safety` 집계에 사용합니다.
- **JOIN vs UNION ALL**: 한 `data_type` 행 안에서 HR·EHS 등 **여러 스테이징의 필드를 합치려면** (예: `safety`) 동일 `(company_id, period_year)`로 파싱 결과를 **JOIN**하거나 애플리케이션에서 아이템을 합친 뒤 한 번에 집계합니다. 반면 **서로 다른 `data_type` 행**은 같은 테이블이지만 **별도 upsert**(별도 자연키)이므로, “한 번에 한 행만 넣는다”는 이전 설명과 달리 **최대 4번의 upsert**가 됩니다.
- **UNION ALL**: `ghg_activity_data`처럼 동질 행을 쌓는 용도와 달리, `social_data`는 **`data_type`별 1행**을 유지하는 것이 목표입니다(중복 행을 UNION으로 늘리지 않음).

```sql
-- 개념: 스테이징별 CTE로 파싱·집계 → data_type마다 별도 INSERT/UPSERT
-- (실제 컬럼 매핑·집계 규칙은 도메인 정의에 따름)
--
-- 1) workforce: HR items만 모아 집계 → INSERT social_data (data_type='workforce')
--    ON CONFLICT (company_id, data_type, period_year) DO UPDATE ...
--
-- 2) supply_chain: SRM items 집계 → data_type='supply_chain' upsert
--
-- 3) safety: HR items + EHS items를 한 집계 함수에 넣어 지표 산출 → data_type='safety' upsert
--
-- 4) community: HR 등에서 공헌 지표만 추출 → data_type='community' upsert
```

**요약**: `staging_hr_data`, `staging_srm_data`, `staging_ehs_data` 등에서 **CTE로 파싱·집계**한 뒤, **`(company_id, data_type, period_year)`** 를 키로 **유형별 upsert**합니다. 이전 문서의 “회사·기간당 1행 + FULL OUTER JOIN 한 방에 조립”은 현재 스키마(`data_type` 분리)와 맞지 않아 본 절에서 정정합니다.
...
2. **ETL 프로세스 구현**
   ...
   - Transform: 스테이징 → 검증 → 최종 변환.
     ...
     - **스테이징(HR/SRM/EHS 등) → social_data**: CTE로 파싱·집계한 뒤 **`(company_id, data_type, period_year)`** 자연키로 **`data_type`별 upsert**(회사·연도당 최대 4행). 동일 `data_type` 내에서만 소스 간 JOIN·머지. 상세는 [옵션 1](#옵션-1-스테이징-테이블-패턴-권장) 내 "스테이징 → social_data 변환" 참고.
...
**역할**: 온프레미스 환경의 사용자 정보 및 권한 관리. [USER_JOURNEY_MAP](./USER_JOURNEY_MAP.md)의 **임원 사전 등록(이름·이메일·권한)** 및 **최종 등록 시 이름·이메일 매칭**과 대응.
...
CREATE TABLE users (
  ...
  name TEXT NOT NULL,  -- 실명. 관리자 사전 등록 시 입력, 임원 최종 등록 시 동일 이름으로 매칭 검증
  
  -- ===== 권한 정보 (관리자가 사전 등록 시 지정, 임원은 읽기 전용 확인) =====
  permission TEXT NOT NULL,  -- 권한(4단계 역할). 'final_approver' | 'esg_team' | 'dept_user' | 'viewer'
  -- 참고: API/레거시 코드에서 컬럼명 `role`을 쓰는 경우 permission과 동일 의미로 매핑 가능
  ...
  INDEX idx_users_name_company (company_id, name),  -- 사전 등록·매칭 조회
  INDEX idx_users_active (company_id, is_active),
  INDEX idx_users_permission (company_id, permission),
...
**역할**: 회사 기본 정보, **온프레미스 회사(테넌트) 계정** 자격 증명, **지주·계열·자회사** 구분, 마감일 관리. 동일 플랫폼을 지주사·계열사·자회사가 함께 쓰는 경우 테넌트(법인) 단위로 `company_login_id`로 구분해 로그인할 수 있다.
...
CREATE TABLE companies (
  ...
  -- ===== 온프레미스 회사(테넌트) 계정 로그인 =====
  -- users.email 과 별개: "회사 계정"으로 플랫폼/관리 콘솔 등에 접속할 때 사용 (정책에 따라 선택적)
  company_login_id TEXT NOT NULL UNIQUE,  -- 회사 계정 로그인 ID
  company_password_hash TEXT NOT NULL,    -- 회사 계정 비밀번호 해시 (bcrypt 등, 평문 저장 금지)
  
  -- ===== 그룹 내 법인 구분 (지주사 / 계열사 / 자회사) =====
  group_entity_type TEXT NOT NULL DEFAULT 'subsidiary' CHECK (
    group_entity_type IN ('holding', 'affiliate', 'subsidiary')
  ),
  -- holding: 지주사, affiliate: 계열사, subsidiary: 자회사
  parent_company_id UUID REFERENCES companies(id) ON DELETE SET NULL,  -- 상위 법인(보통 지주). 지주사는 NULL
  ...
  INDEX idx_companies_login (company_login_id),
  INDEX idx_companies_group_type (group_entity_type),
...
**`group_entity_type` 값**:
- `holding`: 지주사 (그룹 최상위, `parent_company_id`는 보통 NULL)
- `affiliate`: 계열사 (동일 그룹 소속 연계 법인)
- `subsidiary`: 자회사 (출자·지배 관계상 종속 법인)

**회사 계정 vs 사용자 계정**: `company_login_id` / `company_password_hash`는 **법인(테넌트) 단위** 자격 증명이고, 개인 로그인은 `users` 테이블의 `email` / `password_hash`를 사용한다. 온프레미스 정책에 따라 회사 계정은 관리자 전용·API 연동 전용으로만 쓰고 일상 업무는 사용자 계정만 쓰도록 나눌 수 있다.
