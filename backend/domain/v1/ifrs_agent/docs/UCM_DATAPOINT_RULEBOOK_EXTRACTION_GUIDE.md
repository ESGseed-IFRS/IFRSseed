...
#### 3.2.1 보충: 주제별 기준(GRI 305 등)의 「정보 수집 요건」 절번과 `dp_id`

일부 주제별 기준서는 특정 **공개**(예: 공개 305-1) 아래에 **별도 절번호**로 정보 수집 요건을 둔다(예: **2.1**, **2.1.1**, **2.2**, **2.2.5.1**). 이 번호는 **공개 번호 305-2와 무관**하므로, `GRI305-2`(공개 305-2)와 혼동되지 않게 **공개 305-1의 숫자 트리는 `GRI305-1-` 접두**로 묶는다.

- **절 2.1(필수, shall)** → 그룹 `GRI305-1-1`, 하위 `GRI305-1-1-1`(2.1.1), `GRI305-1-1-2`(2.1.2). 부모 공개 DP(`GRI305-1`)의 `child_dps`에 포함.
- **절 2.2(권고, should)** → 그룹 `GRI305-1-2`, 하위 `GRI305-1-2-1` … `GRI305-1-2-5`, 필요 시 `GRI305-1-2-5-1` … `GRI305-1-2-5-4`(2.2.5.1–4).
- `dp_code`는 예: `GRI_305_DIS_1_1_1`, `GRI_305_DIS_1_2_5_3`처럼 **공개 305-1·절 스택**을 드러낸다(원문 절번 2.1·2.2와 대응).
- Rulebook은 `RULE_GRI305_1_1`, `RULE_GRI305_1_1_1`, `RULE_GRI305_1_2`, … 처럼 **동일 절번 스택**을 따른다.

이 패턴은 원문에 **명시된 숫자 하위 항목**(2.1.1, 2.2.5.1 등)이 있을 때만 적용한다. 임의의 `-pct`, `-amount` 접미어로 DP를 쪼개지 않는다는 §3.2.1 원칙과 함께 둔다.

**구분:** 공개 **305-2** 루트는 `GRI305-2`이고 하위 요구사항은 `GRI305-2-a` 등 **문자 항목**과 정보 수집 `GRI305-2-3`·`GRI305-2-4` 등을 쓴다. 공개 305-1의 절 2.1·2.2 트리(`GRI305-1-1`, `GRI305-1-2`)와 ID 공간이 겹치지 않는다.
