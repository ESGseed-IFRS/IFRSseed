"""GRI 2 JSON(datapoint / rulebook)을 DB에 적재하고 BGE-M3 임베딩까지 저장하는 스크립트
...
사용 예:
    python load_gri_2_datapoint_rulebook_embeddings.py
    python load_gri_2_datapoint_rulebook_embeddings.py --force
    python load_gri_2_datapoint_rulebook_embeddings.py --dry-run
    python load_gri_2_datapoint_rulebook_embeddings.py --data-points-only
    python load_gri_2_datapoint_rulebook_embeddings.py --rulebooks-only
"""
...
# backend/domain/v1 (…/ifrs_agent/data/gri_2 → 상위 3단계)
    ...
    for i, rb in enumerate(items, 1):
        ...
        try:
            ...
            record = {
                ...
                "standard_id": rb.get("standard_id", "GRI2"),
...
def main() -> None:
    _configure_logging()
    parser = argparse.ArgumentParser(description="GRI 2 datapoint/rulebook JSON + 임베딩 적재")
    ...
    logger.info("=" * 60)
    logger.info("GRI 2 datapoint.json / rulebook.json → DB + 임베딩")
    logger.info("=" * 60)
