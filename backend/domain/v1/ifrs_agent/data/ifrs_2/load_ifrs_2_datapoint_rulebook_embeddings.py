"""IFRS 2 JSON(datapoint / rulebook)을 DB에 적재하고 BGE-M3 임베딩까지 저장하는 스크립트
...
사용 예:
    python load_ifrs_2_datapoint_rulebook_embeddings.py
    python load_ifrs_2_datapoint_rulebook_embeddings.py --force
    python load_ifrs_2_datapoint_rulebook_embeddings.py --dry-run
    python load_ifrs_2_datapoint_rulebook_embeddings.py --data-points-only
    python load_ifrs_2_datapoint_rulebook_embeddings.py --rulebooks-only
"""
...
# backend/domain/v1 (…/ifrs_agent/data/ifrs_2 → 상위 3단계)
    ...
    for i, dp_data in enumerate(items, 1):
        ...
        try:
            ...
            record = {
                ...
                "standard": dp_data.get("standard", "IFRS"),
    ...
    for i, rb in enumerate(items, 1):
        ...
        try:
            ...
            record = {
                ...
                "standard_id": rb.get("standard_id", "IFRS2"),
...
def main() -> None:
    _configure_logging()
    parser = argparse.ArgumentParser(description="IFRS 2 datapoint/rulebook JSON + 임베딩 적재")
    ...
    logger.info("=" * 60)
    logger.info("IFRS 2 datapoint.json / rulebook.json → DB + 임베딩")
    logger.info("=" * 60)
