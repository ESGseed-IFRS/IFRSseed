...
class SrReportImage(Base):
    ...
    # 페이지 좌표계(PyMuPDF)에서의 배치: [[x0,y0,x1,y1], ...] (동일 이미지 다중 배치 가능)
    placement_bboxes = Column(JSONB, nullable=True)
    # 시맨틱 검색용 1024차원 벡터(pgvector) 및 임베딩 입력 문자열(캡션·VLM 설명 등)
    image_embedding = Column(vector_column(1024), nullable=True)
    image_embedding_text = Column(Text, nullable=True)
