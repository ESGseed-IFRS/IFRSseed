...
class SRAgent:
    ...
    def build_system_prompt(
        ...
        domains = sorted([d for d in (allowed_domains or set()) if isinstance(d, str) and d.strip()])
        guardrail_lines = ""
        if terms:
            guardrail_lines += f'\n추가 검색 키워드(우선 포함): {", ".join(terms[:6])}'
        if domains:
            guardrail_lines += (
                f'\n허용 도메인(반드시 이 중 하나): {", ".join(domains)}'
                "\n허용 도메인이 아닌 URL은 절대 download_pdf_bytes로 호출하지 않는다."
            )
        ...
        for i in range(self.max_iterations):
            ...
            for idx, tc in enumerate(response.tool_calls):
                ...
                if name not in tool_map:
                    ...
                    if (
                        name == "download_pdf_bytes"
                        and isinstance(args, dict)
                        and allowed_domains
                        and not self.tool_utils.is_url_allowed_for_company(str(args.get("url") or ""), allowed_domains)
                    ):
                        tool_result = {
                            "success": False,
                            "error": (
