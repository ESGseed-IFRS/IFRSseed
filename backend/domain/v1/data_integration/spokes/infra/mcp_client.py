...
def _load_sr_body_tools_with_structured_schema() -> List[Any]:
    ...
    class MapBodyPagesInput(BaseModel):
        ...
        use_llm_toc_align: bool = Field(
            ...
            description=(
                "호환용 옵션(현재 미사용). "
                "toc_path·subtitle 후 기본 BGE-M3로 content_embedding(1024) 채움(SR_BODY_METADATA_EMBED=0으로 끔)"
            ),
        )
    ...
    class SaveBodyBatchInput(BaseModel):
        ...
        bodies: list[dict[str, Any]] = Field(
            description=(
                ...
                "is_index_page, content_type, paragraphs, toc_path, subtitle, content_embedding, content_embedding_text, embedding_status 선택"
            ),
        )
        ...
        if server_name == "sr_images_tools":
            ...
            return [
                ...
                _wrap_inprocess_tool(
                    ...
                    "추출 결과를 sr_report_images 저장용 행으로 변환. caption_text 있으면 SR_IMAGE_CAPTION_EMBED 기본 켜짐 시 image_embedding(1024) 채움.",
                ),
                _wrap_inprocess_tool(
                    "save_sr_report_images_batch_tool",
                    _save_sr_report_images_batch_tool,
                    "sr_report_images 배치 저장(image_embedding·image_embedding_text·embedding_status 행에 포함 가능).",
                ),
            ]
