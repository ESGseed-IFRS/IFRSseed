...
에이전트·MCP·비개발자 설명용 **요약·부록 프롬프트**: [`SR_AGENT_LLM_BRIEF.md`](./SR_AGENT_LLM_BRIEF.md).  
본 문서를 대체하지 않으며, **본문·인덱스·이미지 파싱**은 각각 [`../body/SR_BODY_PARSING_LLM_BRIEF.md`](../body/SR_BODY_PARSING_LLM_BRIEF.md), [`../index/SR_INDEX_PARSING_LLM_BRIEF.md`](../index/SR_INDEX_PARSING_LLM_BRIEF.md), [`../images/SR_IMAGES_PARSING_LLM_BRIEF.md`](../images/SR_IMAGES_PARSING_LLM_BRIEF.md) 를 병행한다.
...
├── docs/
│   ├── sr/
│   │   └── data_integration.md    # SR 다운로드 API 본 문서
│   ├── index/                 # 목차·인덱스 파싱·저장 설계
│   ├── images/                # 이미지 파싱·VLM·스토리지
│   ├── body/                  # 본문 파싱 설계
│   ├── Crawling/              # 외부 웹 배치 수집
│   └── PDF_PARSING_IN_MEMORY.md # PDF bytes 파싱 (루트)
└── requirement.txt
