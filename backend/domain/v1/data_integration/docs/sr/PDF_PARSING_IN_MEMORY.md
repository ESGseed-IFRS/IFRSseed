...
> **참고**: 예전 문서의 `parse_sr_report_images` 심볼은 `sr_report_tools`에 없습니다. 이미지는 위 `extract_report_images` + [SR_IMAGES_PARSING_DESIGN.md](./images/SR_IMAGES_PARSING_DESIGN.md) 플로우를 사용하세요.
...
- 환경변수: `SR_IMAGE_MAX_EDGE`(긴 변 축소, 0=비활성), `SR_IMAGE_SKIP_INDEX_PAGES`, `SR_IMAGE_DEBUG` 등은 [SR_IMAGES_PARSING_DESIGN.md](./images/SR_IMAGES_PARSING_DESIGN.md) 참고.
