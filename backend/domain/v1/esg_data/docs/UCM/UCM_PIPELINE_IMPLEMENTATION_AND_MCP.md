...
|---|-------------|---------------------|-----------|
| 1 | **EmbeddingCandidateTool** | §2-1 | 소스 DP·타깃 기준서·`top_k` 입력 → 후보 목록, **`hybrid_score`**, 순위, (선택) 근거. |
...
| 3 | **LLM 재평가** (`UCMCreationAgent.llm_refinement` 등) | §2-3 | 오케스트레이터에서 **`use_llm_in_mapping_service`** 가 참이고 `ucm_policy.should_call_llm`이 참일 때 호출. (현재 `should_call_llm`은 항상 `True`이므로 실질 스위치는 API/요청의 `use_llm_in_mapping_service`.) |
...
[§2-2] RuleValidationTool  (datapoint + rulebook 조인 데이터 입력)
    → 규칙 점수·위반
[§2-3] llm_refinement (use_llm_in_mapping_service 시)
    → LLMRefinementResult
[§2-4] ucm_policy.decide_mapping_pair (에이전트는 policy_finalize_decision으로 래핑)
...
| LLM 재평가 | `UCMCreationAgent.llm_refinement` / `llm_refinement_batch` |
...
| 정책 점수·판정(순수) | `backend/domain/v1/esg_data/spokes/agents/ucm_policy.py` |
| Agent 퍼사드 | `backend/domain/v1/esg_data/spokes/agents/ucm_creation_agent.py` (`policy_pick_best`, `policy_finalize_decision`) |
...
- **Agent는 정책만 실행한다.** §5 점수식은 `ucm_policy.compute_final_score`에 구현되어 있고, 치명 위반·`pick_best` 제외·최종 판정은 `ucm_policy` + `policy_finalize_decision`이 담당한다. LLM은 오케스트레이터의 **`use_llm_in_mapping_service`** 와 `should_call_llm`(현재 항상 `True`) 조합으로 게이트된다.

### 4.2 공개 API (문서 §6·코드와 정렬)

- **`ucm_policy`**: `compute_final_score`, `compute_penalty`, `tentative_decision_from_scores`, `should_call_llm`, `pick_best_candidate_pair`, `decide_mapping_pair`, `diagnose_pick_best_failures`
- **`UCMCreationAgent`**: `policy_pick_best`, `policy_finalize_decision`, `llm_refinement`, `llm_refinement_batch`

`DecisionResult` 필드: `decision`, `confidence`, `final_score`, `reason_codes`, `llm_used`, `evidence`, `chosen_target_dp_id` 등 (`ucm_pipeline_contracts`).
...
- **레거시 배치** `UCMMappingService.create_mappings` / MCP `create_unified_column_mapping`은 **스텁(항상 error)** 이다. 실데이터 반영은 **`run_ucm_policy_pipeline` / `run_ucm_nearest_pipeline`** 경로를 쓴다. 자세한 비교는 [UCM_POLICY_PIPELINE_AND_LEGACY_BATCH.md](./UCM_POLICY_PIPELINE_AND_LEGACY_BATCH.md) 참고.
...
**구현 반영(코드)**: 계약 `ucm_pipeline_contracts.py`; LangGraph 상태 `ucm_workflow_state.py`; Tool 3종 `shared/tool/UnifiedColumnMapping/`; 정책 `ucm_policy.py` + `UCMCreationAgent.llm_refinement*`; `UCMOrchestrator.run_ucm_policy_pipeline` / `_nearest`; API **`POST /esg-data/ucm/pipeline/policy`** (및 nearest). ESG UCM MCP는 **in-process·stdio** 위주이며, 정책 파이프라인은 FastAPI·오케스트레이터가 공유 Tool을 직접 호출하는 경로가 기본이다.
