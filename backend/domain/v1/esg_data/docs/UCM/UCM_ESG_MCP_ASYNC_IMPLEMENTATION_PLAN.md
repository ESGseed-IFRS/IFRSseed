...
- **3단계**: `UCMCreationAgent.acreate_mappings` / `UCMOrchestrator.arun_validation_step`는 `to_thread` + `DirectEsgToolRuntime`; `UCMOrchestrator`의 `*_async` 메서드, FastAPI `ucm_router`는 `await *_async`. LangGraph 경로는 **`ainvoke` + 비동기 노드**(create/validate) 우선, 실패 시 동기 `invoke`를 스레드로 폴백. 정책 파이프라인 LLM은 **`use_llm_in_mapping_service`**(요청 기본 `false`)와 `ucm_policy.should_call_llm`(현재 항상 `True`) 조합으로 제어된다.
