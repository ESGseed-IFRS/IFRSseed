    ...
    def upsert_ucm_from_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing is not None:
                if not _check_cluster_consistency(existing, payload):
                    logger.warning(
                        "UCM merge 일관성 실패 — 별도 row 생성: existing={}, incoming={}",
                        existing.unified_column_id,
                        uid,
                    )
                    existing = None
                else:
...
def _check_cluster_consistency(existing: UnifiedColumnMapping, incoming_payload: dict[str, Any]) -> bool:
    """병합 전 기존 cluster와 incoming의 의미 일관성을 확인한다."""
    ex_meta = getattr(existing, "standard_metadata", None)
    in_meta = incoming_payload.get("standard_metadata")
    if not isinstance(ex_meta, dict) or not isinstance(in_meta, dict):
        return True
    ex_topics = {str(v.get("topic", "")).strip().lower() for v in ex_meta.values() if isinstance(v, dict)}
    in_topics = {str(v.get("topic", "")).strip().lower() for v in in_meta.values() if isinstance(v, dict)}
    ex_topics.discard("")
    in_topics.discard("")
    if ex_topics and in_topics and not (ex_topics & in_topics):
        return False
    ex_subtopics = {str(v.get("subtopic", "")).strip().lower() for v in ex_meta.values() if isinstance(v, dict)}
    in_subtopics = {str(v.get("subtopic", "")).strip().lower() for v in in_meta.values() if isinstance(v, dict)}
    ex_subtopics.discard("")
    in_subtopics.discard("")
    if ex_topics and in_topics and ex_subtopics and in_subtopics and not (ex_subtopics & in_subtopics):
        return False
    return True
