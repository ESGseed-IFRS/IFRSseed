...
from backend.domain.shared.tool.UnifiedColumnMapping.ucm_dp_mapping_signals import is_leaf_dp
...
_UCM_TRACE_JSON_MAX = 14_000


def _ucm_trace_dumps(obj: Any) -> str:
    try:
        s = json.dumps(obj, ensure_ascii=False, default=str)
    except Exception:
        s = str(obj)
    if len(s) > _UCM_TRACE_JSON_MAX:
        return s[:_UCM_TRACE_JSON_MAX] + "…(truncated)"
    return s
        ...
        *,
        ...
        try:
            ...
            def _finalize_policy_row(
                *,
                ...
                decision = self.creation_agent.policy_finalize_decision(
                    ...
                    rule_row=rule_row,  # type: ignore[arg-type]
                ...
                if target is None:
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_policy_v1 | stage=finalize | source_dp_id={} | message=target_dp_not_found | "
                        "target_dp_id={} | decision={} | rule_violations={}",
                        src_id,
                        candidate.get("target_dp_id"),
                        decision.get("decision"),
                        _ucm_trace_dumps(rule_row.get("violations")),
                    )
                    items.append(
                        {
                            "source_dp_id": src_id,
                ...
                items.append(row)
                logger.info(
                    "UCM trace | pipeline=ucm_policy_v1 | stage=finalized | detail={}",
                    _ucm_trace_dumps(
                        {
                            "source_dp_id": src_id,
                            "target_dp_id": candidate["target_dp_id"],
                            ...
                            "upsert_result": {
                                "status": (upsert_result or {}).get("status"),
                                "mode": (upsert_result or {}).get("mode"),
                                "message": (upsert_result or {}).get("message"),
                            },
                            "rule_row": {
                                "rule_pass": rule_row.get("rule_pass"),
                                "rule_score": rule_row.get("rule_score"),
                                "structure_score": rule_row.get("structure_score"),
                                "violations": rule_row.get("violations"),
                            },
                            "llm_result": llm_result,
                        }
                    ),
            ...
            for source in source_rows:
                if mapping_leaf_only and not is_leaf_dp(source):
                    stats["skipped_non_leaf_source"] += 1
                    continue
                stats["processed"] += 1
                src_id = source.dp_id
                ...
                emb = embedding_tool.run(
                    ...
                    structural_threshold=structural_threshold,
                    final_threshold=final_threshold,
                    leaf_targets_only=mapping_leaf_only,
                )
                if emb["status"] != "success" or not emb["candidates"]:
                    stats["reject"] += 1
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_policy_v1 | stage=embedding | source_dp_id={} | emb_status={} | "
                        "candidate_count={} | top_k={} | vector_threshold={} | structural_threshold={} | final_threshold={} | message={}",
                        src_id,
                        emb.get("status"),
                        len(emb.get("candidates") or []),
                        top_k,
                        vector_threshold,
                        structural_threshold,
                        final_threshold,
                        emb.get("message") or "no candidates",
                    )
                    items.append(
                        {
                            "source_dp_id": src_id,
                ...
                if rv["status"] != "success" or not rv["per_candidate"]:
                    ...
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_policy_v1 | stage=rule | source_dp_id={} | rule_status={} | "
                        "per_candidate_len={} | message={} | detail={}",
                        src_id,
                        rv.get("status"),
                        len(rv.get("per_candidate") or []),
                        rv.get("message", "rule validation failed"),
                        _ucm_trace_dumps(rv.get("per_candidate") or []),
                    )
                    items.append(
                        {
                            "source_dp_id": src_id,
                ...
                if best is None:
                    ...
                    stats["reject"] += 1
                    diag = ucm_policy.diagnose_pick_best_failures(emb["candidates"], rv["per_candidate"])
                    logger.info(
                        "UCM trace | pipeline=ucm_policy_v1 | stage=pick_best | source_dp_id={} | message=no_valid_pair | diagnosis={}",
                        src_id,
                        _ucm_trace_dumps(diag),
                    )
                ...
                if use_llm_in_mapping_service and ucm_policy.should_call_llm(
                    ...
                    src_dpv = rule_evidence.get("source_datapoint_validation_rules", []) if isinstance(rule_evidence, dict) else []
                    tgt_dpv = rule_evidence.get("target_datapoint_validation_rules", []) if isinstance(rule_evidence, dict) else []
                    para_ax = rule_evidence.get("paragraph_axis", {}) if isinstance(rule_evidence, dict) else {}
                    pending_llm.append(
                        {
                            "source": source,
                            "src_id": src_id,
                            ...
                            "target_snapshot": {
                                "name_ko": getattr(target_preview, "name_ko", None),
                                "name_en": getattr(target_preview, "name_en", None),
                                "topic": getattr(target_preview, "topic", None),
                                "subtopic": getattr(target_preview, "subtopic", None),
                                "description": getattr(target_preview, "description", None),
                                "standard": getattr(target_preview, "standard", None),
                            },
                                "candidate": candidate,
                                "rule_row": rule_row,
                                "validation_rules": validation_rules,
                                "source_datapoint_validation_rules": src_dpv,
                                "target_datapoint_validation_rules": tgt_dpv,
                                "paragraph_axis": para_ax,
                                "tentative_decision": tentative,
                            },
                        }
                    )
                    if len(pending_llm) >= llm_micro_batch_size:
                        contexts = [p["context"] for p in pending_llm]
                        llm_results = self.creation_agent.llm_refinement_batch(contexts, model=llm_model)
                        # LLM 대기 중 DB 연결이 idle로 끊기지 않도록 세션 교체 후 소스 재조회
                        if db is not None:
                            db.close()
                        db = get_session()
                        for p, lr in zip(pending_llm, llm_results):
                            fresh_src = (
                                db.query(DataPoint)
                                .filter(DataPoint.dp_id == p["src_id"], DataPoint.is_active.is_(True))
                                .first()
                            )
                            if fresh_src is None:
                                ...
                                items.append(
                                    {
                                        "source_dp_id": p["src_id"],
                                        "status": "error",
                                        "message": "source dp not found after db reconnect",
                                    }
                                )
                                continue
                            _finalize_policy_row(
                                source=fresh_src,
                                src_id=p["src_id"],
        ...
        try:
            ...
            def _finalize_nearest_row(
                *,
                ...
                if not target:
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_nearest_v1 | stage=finalize | source_dp_id={} | message=target_dp_not_found | "
                        "target_dp_id={} | decision={} | rule_violations={}",
                        src_id,
                        candidate.get("target_dp_id"),
                        decision.get("decision"),
                ...
                items.append(row_n)
                logger.info(
                    "UCM trace | pipeline=ucm_nearest_v1 | stage=finalized | detail={}",
                    _ucm_trace_dumps(
                        {
                            "source_dp_id": src_id,
                            "target_dp_id": candidate["target_dp_id"],
            ...
            for source in sources:
                ...
                if emb["status"] != "success" or not emb["candidates"]:
                    ...
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_nearest_v1 | stage=embedding | source_dp_id={} | emb_status={} | "
                        "candidate_count={} | top_k={} | vector_threshold={} | structural_threshold={} | final_threshold={} | message={}",
                        src_id,
                        emb.get("status"),
                        len(emb.get("candidates") or []),
                ...
                if rv["status"] != "success" or not rv["per_candidate"]:
                    ...
                    stats["errors"] += 1
                    logger.info(
                        "UCM trace | pipeline=ucm_nearest_v1 | stage=rule | source_dp_id={} | rule_status={} | "
                        "per_candidate_len={} | message={} | detail={}",
                        src_id,
                        rv.get("status"),
                        len(rv.get("per_candidate") or []),
                ...
                if best is None:
                    ...
                    diag = ucm_policy.diagnose_pick_best_failures(emb["candidates"], rv["per_candidate"])
                    logger.info(
                        "UCM trace | pipeline=ucm_nearest_v1 | stage=pick_best | source_dp_id={} | message=no_valid_pair | diagnosis={}",
                        src_id,
                        _ucm_trace_dumps(diag),
                    )
        ...
        *,
        ...
        return await asyncio.to_thread(
            lambda: self.run_ucm_policy_pipeline(
                ...
                llm_model=llm_model,
                persist_mode=persist_mode,
                mapping_leaf_only=mapping_leaf_only,
            ),
        )
