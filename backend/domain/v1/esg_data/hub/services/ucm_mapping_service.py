...
class UCMMappingService:
    ...
    def find_similar_dps_hybrid(
        ...
        *,
        ...
        final_threshold: float = 0.75,
        top_k: int = 5,
        leaf_targets_only: bool = True,
        db: Session | None = None,
    ) -> list[dict[str, Any]]:
        """UCM 툴용 단순 인프로세스 하이브리드 검색 폴백.

        ``db``가 있으면 해당 세션만 사용하고 닫지 않는다(오케스트레이터 장시간 idle 연결 방지).
        """
        _ = vector_threshold
        owns_db = db is None
        if owns_db:
        ...
        try:
            ...
            for target in targets:
                if leaf_targets_only and not is_leaf_dp(target):
                    continue
        ...
        try:
            ...
            if getattr(source, "embedding", None) is None:
                return self._cross_standard_structural_fallback(
                    ...
                    top_k=top_k,
                    leaf_targets_only=leaf_targets_only,
                )

            # pgvector 거리: 작을수록 가깝다. 대략 [0,1] 유사도로 변환.
            leaf_clause = ""
            if leaf_targets_only:
                leaf_clause = " AND coalesce(array_length(t.child_dps, 1), 0) = 0 "
            sql = f"""
