    ...
    *,
    ...
    if llm_result and llm_result.get("status") == "success" and llm_result.get("refinement_score") is not None:
        ...
        if llm_decision == "reject":
            ...
            evidence["llm_decision_override"] = "reject"
            evidence["llm_refinement_score"] = rs
        else:
            final_score = max(0.0, min(1.0, 0.35 * final_score + 0.65 * rs))
            evidence["llm_refinement_score"] = rs
            tentative = tentative_decision_from_scores(final_score, has_critical)
            if llm_decision in {"accept", "review"} and not has_critical:
...
def diagnose_pick_best_failures(
    candidates: List[EmbeddingCandidateItem],
    per_rule: List[RuleCandidateResult],
) -> List[Dict[str, Any]]:
    """``pick_best``가 None일 때 로그/API 추적용: 후보별 점수·위반·탈락 사유."""
    rows: List[Dict[str, Any]] = []
    for cand in sorted(candidates, key=lambda c: float(c["hybrid_score"]), reverse=True):
        rr = merge_candidate_rule(cand, per_rule)
        tid = cand.get("target_dp_id")
        if rr is None:
            rows.append(
                {
                    "target_dp_id": tid,
                    "skip_reason": "no_matching_rule_row",
        ...
        rows.append(
            {
                "target_dp_id": tid,
                "hybrid_score": round(float(cand["hybrid_score"]), 6),
                "rule_score": rr["rule_score"],
                "structure_score": rr["structure_score"],
                "requirement_score": rr["requirement_score"],
                "penalty": round(penalty, 4),
                "final_score": round(fs, 4),
                "rule_pass": rr["rule_pass"],
                "has_critical": has_critical,
                "pick_best_skipped": has_critical,
                "violations": [dict(v) for v in violations],
            }
        )
    return rows
