...
from .sr_image_caption_embedding import enrich_image_rows_with_caption_embeddings
...
def map_extracted_images_to_sr_report_rows(
    ...
    Args:
        ...
        images_by_page: { page_number: [ { path 또는 image_bytes, width, height, size_bytes, image_index, mime_type, placement_bboxes, caption_text }, ... ] }
        반환 전 caption_text가 있으면 SR_IMAGE_CAPTION_EMBED로 BGE-M3 image_embedding을 채웁니다.
    ...
    for page_number in sorted(images_by_page.keys()):
        for item in images_by_page[page_number]:
            ...
            pb = item.get("placement_bboxes")
            if pb is not None:
                row["placement_bboxes"] = pb
