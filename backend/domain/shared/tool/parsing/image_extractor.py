...
def _placement_bboxes_for_xref(page: Any, xref: int) -> Optional[List[List[float]]]:
    """
    PyMuPDF 페이지 좌표계(포인트, 좌상단 원점)에서 이미지 xref의 배치 사각형들.
    반환: [[x0, y0, x1, y1], ...] — 동일 이미지가 여러 영역에 있으면 길이 > 1.
    """
    fn = getattr(page, "get_image_rects", None)
    if not callable(fn):
        return None
    try:
        rects = fn(xref)
...
def extract_report_images(
    ...
    *,
    ...
    Returns:
        {
            "success": bool,
            "images_by_page": { page_number: [ { "path", "width", "height", "size_bytes", "image_index", "placement_bboxes" }, ... ] },
            "error": str | None,
        ...
        for pn in pages:
            ...
            for img in imglist:
                ...
                page_entries.append({
                    ...
                    "placement_bboxes": _placement_bboxes_for_xref(page, xref),
                })
                local_idx += 1
