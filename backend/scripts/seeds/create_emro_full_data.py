...
def create_ems_data(base_dir: Path):
    ...
    # 1. EMS_ENERGY_USAGE.csv (이미 수정된 파일 유지 - GHG 산정 필드 포함)
    # 기존 파일을 읽어서 그대로 유지
    pass
    ...
    with open(ems_dir / "ENV_WASTE_DETAIL.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        waste_types = [
            ('일반폐기물', '종이류', 1.8, 1.7, 0.1, 0, 94.4, 'N'),
            ('일반폐기물', '플라스틱', 0.9, 0.7, 0.2, 0, 77.8, 'N'),
            ('일반폐기물', '음식물', 3.2, 0, 0, 3.2, 0, 'N'),
            ('전자폐기물', 'IT장비', 0.5, 0.5, 0, 0, 100, 'Y'),
        ]
        
        for q in range(1, 5):
            for idx, (cat, subcat, gen, rec, land, inc, rec_rate, haz) in enumerate(waste_types):
                record_id = f"WST-ML-2024-Q{q}-{idx+1:03d}"
    ...
    with open(ems_dir / "ENV_WATER_USAGE.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        site = COMPANY['sites'][0]
        base_monthly_m3 = 180  # 250명 × 0.72 m³/인/월
        
        for month in range(1, 13):
            record_id = f"WTR-ML-2024-{month:04d}"
            monthly_m3 = base_monthly_m3 * (1.1 if month in [7, 8] else 1.0)
...
def create_erp_data(base_dir: Path):
    ...
    with open(erp_dir / "FIN_REVENUE.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        quarterly_revenue = 8_500_000_000  # 분기당 85억원
        
        for q in range(1, 5):
            record_id = f"REV-ML-2024-Q{q}"
            operating_profit = int(quarterly_revenue * 0.15)
            net_profit = int(quarterly_revenue * 0.11)
    ...
    with open(erp_dir / "FIN_CAPEX.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        for q in range(1, 5):
            record_id = f"CAPEX-ML-2024-Q{q}"
            total_capex = 1_200_000_000  # 분기당 12억원
            rd_capex = int(total_capex * 0.60)
...
def create_hr_data(base_dir: Path):
    ...
    with open(hr_dir / "HR_HEADCOUNT.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        for month in range(1, 13):
            record_id = f"HC-ML-2024-{month:04d}"
            total_hc = base_hc + (month // 4)
            male = int(total_hc * 0.58)
            female = total_hc - male
            new_hire = 3 if month % 3 == 1 else 1
            resignation = 2 if month % 6 == 0 else 0
            
            writer.writerow([
                record_id, site['code'], site['name'], 2024, month,
                total_hc, int(total_hc * 0.92), int(total_hc * 0.08), male, female,
    ...
    with open(hr_dir / "HR_TRAINING.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        for q in range(1, 5):
            record_id = f"TRN-ML-2024-Q{q}"
            total_hours = 250 * 16  # 1인당 분기 16시간
            cost = 35_000_000  # 분기당 3,500만원
...
def create_ehs_data(base_dir: Path):
    ...
    with open(ehs_dir / "EHS_SAFETY_KPI.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        for q in range(1, 5):
            record_id = f"SAFETY-ML-2024-Q{q}"
            incidents = 1 if q == 2 else 0
            near_misses = 2 if q in [1, 3] else 1
            training_hours = 250 * 4
    ...
    with open(mdg_dir / "MDG_SITE_MASTER.csv", 'w', newline='', encoding='utf-8') as f:
        ...
        for site in COMPANY['sites']:
            writer.writerow([
                ...
                site['floor_area_m2'], site['employees'], 'ACTIVE', '2018-03-15',
