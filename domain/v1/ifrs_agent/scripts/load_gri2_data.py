"""GRI2 data_point.json과 rulebook.json 데이터를 데이터베이스에 로드하는 스크립트

이 스크립트는 다음 작업을 수행합니다:
1. ai/ifrs_agent/data/gri2/data_point.json 파일에서 데이터 포인트를 읽어옴
2. ai/ifrs_agent/data/gri2/rulebook.json 파일에서 rulebook 섹션을 읽어옴
3. data_points 테이블과 rulebooks 테이블에 데이터 저장 (upsert 방식)

사용법:
    python load_gri2_data.py                    # 두 파일 모두 로드
    python load_gri2_data.py --data-points-only # data_point.json만 로드
    python load_gri2_data.py --rulebooks-only   # rulebook.json만 로드
    python load_gri2_data.py --force            # 기존 데이터 덮어쓰기
    python load_gri2_data.py --dry-run          # 실제 저장 없이 테스트
"""
import sys
import os
import json
...
def generate_embedding_text_for_rulebook(rulebook_data: Dict[str, Any]) -> str:
    ...
    if rulebook_data.get("rulebook_content") or rulebook_data.get("section_content"):
        # section_content 또는 rulebook_content 사용
        content = rulebook_data.get("rulebook_content") or rulebook_data.get("section_content", "")
        # 내용이 너무 길면 앞부분만 사용
        if len(content) > 2000:
...
def load_data_points_json() -> List[Dict[str, Any]]:
    ...
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "gri2" / "data_point.json"
...
def map_dp_type(dp_type_str: Optional[str]) -> str:
    ...
    type_mapping = {
        ...
        "narrative": "narrative",
        "binary": "binary",
        "numeric": "quantitative",  # numeric을 quantitative로 매핑
        "QUANTITATIVE": "quantitative",
        "QUALITATIVE": "qualitative",
        "NARRATIVE": "narrative",
        "BINARY": "binary",
...
def map_unit(unit_str: Optional[str]) -> Optional[str]:
    ...
    unit_mapping = {
        ...
        "text": "text",
        "인원": "count",  # 인원을 count로 매핑
        "ratio": "percentage",  # ratio를 percentage로 매핑
        "비율": "percentage"  # 비율을 percentage로 매핑
    }
    mapped = unit_mapping.get(unit_str.lower(), None)
    if not mapped:
        # 매핑되지 않은 단위는 None으로 설정 (경고 로그)
        logger.warning(f"알 수 없는 unit 값: '{unit_str}', NULL로 설정")
    return mapped
            ...
            try:
                ...
                if parent_indicator:
                    ...
                    if parent_indicator == "":
                        ...
                        # 현재 배치에서 존재하는지 확인 (정렬된 순서이므로 앞에 있는 것들은 이미 처리됨)
                        ...
                        # 부모가 배치에 있지만 DB에 없으면, 부모가 실패했을 가능성이 높으므로 NULL로 설정
                        # 부모가 배치에 없고 DB에도 없으면, 표준 ID나 섹션 ID일 수 있으므로 NULL로 설정
                        if not parent_exists_in_db:
                            if parent_exists_in_batch:
                                logger.debug(f"[{i}/{total}] {dp_id}: parent_indicator '{parent_indicator}'가 배치에 있지만 DB에 없어 NULL로 설정 (부모가 실패했을 가능성)")
                            else:
                                logger.debug(f"[{i}/{total}] {dp_id}: parent_indicator '{parent_indicator}'가 존재하지 않아 NULL로 설정 (표준/섹션 ID일 수 있음)")
                            parent_indicator = None
                ...
                # 4. dp_type 매핑 및 검증
                dp_type_raw = dp_data.get("dp_type", "narrative")
                dp_type = map_dp_type(dp_type_raw)
                if dp_type_raw != dp_type:
                    logger.debug(f"[{i}/{total}] {dp_id}: dp_type '{dp_type_raw}'를 '{dp_type}'로 매핑")
                
                # 5. unit 매핑 및 검증
                unit_raw = dp_data.get("unit")
                unit = map_unit(unit_raw)
                if unit_raw and unit_raw != unit:
                    logger.debug(f"[{i}/{total}] {dp_id}: unit '{unit_raw}'를 '{unit}'로 매핑")
                ...
                dp_record = {
                    ...
                    "dp_type": dp_type,  # 매핑된 dp_type 사용
                    "unit": unit,  # 매핑된 unit 사용
...
def load_rulebooks_json() -> List[Dict[str, Any]]:
    ...
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "gri2" / "rulebook.json"
...
def check_existing_rulebook(cursor, rulebook_id: str) -> bool:
    """기존 Rulebook 존재 여부 확인 (rulebook_id로 확인)"""
    cursor.execute(
        "SELECT 1 FROM rulebooks WHERE rulebook_id = %s AND is_active = TRUE",
        (rulebook_id,)
    )
    return cursor.fetchone() is not None


def get_existing_rulebook_id(cursor, rulebook_id: str) -> Optional[str]:
    """기존 Rulebook의 rulebook_id 가져오기"""
    cursor.execute(
        "SELECT rulebook_id FROM rulebooks WHERE rulebook_id = %s AND is_active = TRUE",
        (rulebook_id,)
    )
...
def load_rulebooks_to_db(
    ...
    try:
        ...
        for i, rulebook_data in enumerate(rulebooks, 1):
            # GRI2 rulebook.json에는 rulebook_id가 이미 포함되어 있음
            ...
            if not rulebook_id:
                ...
                if not standard_id or not section_name:
                    logger.warning(f"[{i}/{total}] rulebook_id, standard_id 또는 section_name이 없어 스킵")
                    stats["skipped"] += 1
                    continue
                
                # rulebook_id 생성
                section_name_clean = section_name.replace(" ", "_").replace("/", "_").replace("(", "").replace(")", "").replace("-", "_")
            ...
            try:
                ...
                existing_id = None
                if cursor and not force_update:
                    if check_existing_rulebook(cursor, rulebook_id):
                        existing_id = get_existing_rulebook_id(cursor, rulebook_id)
                        if existing_id:
                            logger.debug(f"[{i}/{total}] {rulebook_id}: 이미 존재, 스킵")
                            stats["skipped"] += 1
                            continue
                elif cursor:
                    if check_existing_rulebook(cursor, rulebook_id):
                ...
                if embedder:
                    try:
                        ...
                        if not section_embedding_text.strip():
                            logger.warning(f"[{i}/{total}] {rulebook_id}: 임베딩 텍스트가 비어있음")
                        ...
                        logger.warning(f"[{i}/{total}] {rulebook_id}: 임베딩 생성 실패 - {e}")
                        section_embedding_text = None
                        section_embedding_list = None
                ...
                rulebook_record = {
                    ...
                    "standard_id": standard_id or rulebook_data.get("standard_id", ""),
                    "section_name": section_name or rulebook_data.get("section_name", ""),
                ...
                if dry_run:
                    ...
                    logger.info(f"[{i}/{total}] {rulebook_id}: {action} (dry-run)")
                    if existing_id:
                        stats["updated"] += 1
                    else:
                        stats["inserted"] += 1
                ...
                if existing_id:
                    ...
                    stats["updated"] += 1
                    logger.debug(f"[{i}/{total}] {rulebook_id}: 업데이트 완료")
                else:
                    insert_rulebook(cursor, rulebook_record)
                    stats["inserted"] += 1
                    logger.debug(f"[{i}/{total}] {rulebook_id}: 삽입 완료")
                ...
                logger.error(f"[{i}/{total}] {rulebook_id}: 처리 실패 - {e}")
                stats["errors"] += 1
                if conn:
                    conn.rollback()
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GRI2 data_point.json과 rulebook.json 데이터를 데이터베이스에 로드"
    )
    parser.add_argument(
        "--data-points-only",
        action="store_true",
