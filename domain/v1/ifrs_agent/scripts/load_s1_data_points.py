"""s1.json 데이터를 data_points 테이블에 로드하고 임베딩을 생성하는 스크립트

이 스크립트는 다음 작업을 수행합니다:
1. ai/ifrs_agent/data/s1/s1.json 파일에서 데이터 포인트를 읽어옴
2. EmbeddingTextService를 사용하여 임베딩 텍스트 생성
3. EmbeddingService를 사용하여 벡터 임베딩 생성
4. data_points 테이블에 데이터 저장 (upsert 방식)

사용법:
    python load_s1_data_points.py
    python load_s1_data_points.py --force  # 기존 데이터 덮어쓰기
    python load_s1_data_points.py --dry-run  # 실제 저장 없이 테스트
"""
import sys
import os
import json
...
def load_s1_json() -> List[Dict[str, Any]]:
    """s1.json 파일에서 데이터 포인트 로드
    
    Returns:
        데이터 포인트 딕셔너리 리스트
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "s1" / "s1.json"
    
    if not json_path.exists():
        logger.error(f"s1.json 파일을 찾을 수 없습니다: {json_path}")
        raise FileNotFoundError(f"s1.json not found at {json_path}")
    
    logger.info(f"s1.json 파일 로드 중: {json_path}")
    ...
    if isinstance(data, dict) and "data_points" in data:
        ...
        logger.error("s1.json 형식이 올바르지 않습니다")
        raise ValueError("Invalid s1.json format")
        ...
        for i, dp_data in enumerate(data_points, 1):
            ...
            try:
                ...
                dp_record = {
                    ...
                    "standard": dp_data.get("standard", "IFRS_S1"),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="s1.json 데이터를 data_points 테이블에 로드하고 임베딩 생성"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("s1.json -> data_points 테이블 로드 스크립트")
    logger.info("=" * 60)
