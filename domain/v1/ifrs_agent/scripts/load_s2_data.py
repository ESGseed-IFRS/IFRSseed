"""IFRS S2 data_point.json과 rulebook.json 데이터를 데이터베이스에 로드하는 스크립트

이 스크립트는 다음 작업을 수행합니다:
1. ai/ifrs_agent/data/s2/data_point.json 파일에서 데이터 포인트를 읽어옴
2. ai/ifrs_agent/data/s2/rulebook.json 파일에서 rulebook 섹션을 읽어옴
3. data_points 테이블과 rulebooks 테이블에 데이터 저장 (upsert 방식)

사용법:
    python load_s2_data.py                    # 두 파일 모두 로드
    python load_s2_data.py --data-points-only # data_point.json만 로드
    python load_s2_data.py --rulebooks-only   # rulebook.json만 로드
    python load_s2_data.py --force            # 기존 데이터 덮어쓰기
    python load_s2_data.py --dry-run          # 실제 저장 없이 테스트
"""
import sys
import os
import json
...
def load_data_points_json() -> List[Dict[str, Any]]:
    ...
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "s2" / "data_point.json"
...
def load_rulebooks_json() -> List[Dict[str, Any]]:
    ...
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "s2" / "rulebook.json"
...
def check_existing_rulebook(cursor, standard_id: str, section_name: str) -> bool:
    """기존 Rulebook 존재 여부 확인 (standard_id, section_name 조합으로 확인)"""
    cursor.execute(
...
def get_existing_rulebook_id(cursor, standard_id: str, section_name: str) -> Optional[str]:
    """기존 Rulebook의 rulebook_id 가져오기 (standard_id, section_name 조합으로 확인)"""
    cursor.execute(
...
def load_rulebooks_to_db(
    ...
    try:
        ...
        for i, rulebook_data in enumerate(rulebooks, 1):
            ...
            # rulebook_id 생성 (standard_id + section_name 기반)
            ...
            try:
                # 1. 기존 레코드 확인 (standard_id, section_name 조합으로 확인)
                existing_id = None
                if cursor and not force_update:
                    if check_existing_rulebook(cursor, standard_id, section_name):
                        ...
                        if existing_id:
                            logger.debug(f"[{i}/{total}] {rulebook_id}: 이미 존재 (standard_id={standard_id}, section_name={section_name}), 스킵")
                            stats["skipped"] += 1
                            continue
                elif cursor:
                    if check_existing_rulebook(cursor, standard_id, section_name):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="IFRS S2 data_point.json과 rulebook.json 데이터를 데이터베이스에 로드"
    )
    parser.add_argument(
        "--data-points-only",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("IFRS S2 데이터 로드 스크립트")
    logger.info("=" * 60)
