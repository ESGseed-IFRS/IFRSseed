"""IFRS S1 rulebooks.json 데이터를 rulebooks 테이블에 로드하고 임베딩을 생성하는 스크립트

이 스크립트는 다음 작업을 수행합니다:
1. ai/ifrs_agent/data/s1/rulebooks.json 파일에서 rulebook 섹션을 읽어옴
2. EmbeddingTextService를 사용하여 임베딩 텍스트 생성
3. EmbeddingService를 사용하여 벡터 임베딩 생성
4. rulebooks 테이블에 데이터 저장 (upsert 방식)

사용법:
    python load_s1_rulebooks.py
    python load_s1_rulebooks.py --force  # 기존 데이터 덮어쓰기
    python load_s1_rulebooks.py --dry-run  # 실제 저장 없이 테스트

주의: 이 스크립트는 IFRS S1 전용입니다. IFRS S2는 load_s2_rulebooks.py, GRI는 load_gri_rulebooks.py를 사용하세요.
"""
import sys
import os
import json
...
def load_s1_rulebooks_json() -> List[Dict[str, Any]]:
    """IFRS S1 rulebooks.json 파일에서 rulebook 섹션 로드
    
    Returns:
        rulebook 섹션 딕셔너리 리스트
    """
    # 파일 경로 설정 (IFRS S1 전용)
    json_path = script_dir.parent / "data" / "s1" / "rulebooks.json"
    
    if not json_path.exists():
        logger.error(f"IFRS S1 rulebooks.json 파일을 찾을 수 없습니다: {json_path}")
        raise FileNotFoundError(f"IFRS S1 rulebooks.json not found at {json_path}")
    
    logger.info(f"IFRS S1 rulebooks.json 파일 로드 중: {json_path}")
    ...
    if isinstance(data, dict) and "rulebooks" in data:
        ...
        logger.error("IFRS S1 rulebooks.json 형식이 올바르지 않습니다")
        raise ValueError("Invalid IFRS S1 rulebooks.json format")
    
    logger.info(f"{len(rulebooks)}개의 IFRS S1 rulebook 섹션 로드 완료")
    return rulebooks
...
def load_rulebooks_to_db(
    ...
    try:
        total = len(rulebooks)
        logger.info(f"총 {total}개 IFRS S1 rulebook 섹션 처리 시작")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="IFRS S1 rulebooks.json 데이터를 rulebooks 테이블에 로드하고 임베딩 생성"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("IFRS S1 rulebooks.json -> rulebooks 테이블 로드 스크립트")
    logger.info("=" * 60)
    ...
    try:
        # 1. JSON 파일 로드
        rulebooks = load_s1_rulebooks_json()
