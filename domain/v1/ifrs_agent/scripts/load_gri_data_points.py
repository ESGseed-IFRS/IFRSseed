"""GRI.json 데이터를 data_points 테이블에 로드하고 임베딩을 생성하는 스크립트

이 스크립트는 다음 작업을 수행합니다:
1. ai/ifrs_agent/data/gri/GRI.json 파일에서 데이터 포인트를 읽어옴
2. EmbeddingTextService를 사용하여 임베딩 텍스트 생성
3. EmbeddingService를 사용하여 벡터 임베딩 생성
4. data_points 테이블에 데이터 저장 (upsert 방식)

사용법:
    python load_gri_data_points.py
    python load_gri_data_points.py --force  # 기존 데이터 덮어쓰기
    python load_gri_data_points.py --dry-run  # 실제 저장 없이 테스트
"""
import sys
import os
import json
...
def load_gri_json() -> List[Dict[str, Any]]:
    """GRI.json 파일에서 데이터 포인트 로드
    
    Returns:
        데이터 포인트 딕셔너리 리스트
    """
    # 파일 경로 설정
    json_path = script_dir.parent / "data" / "gri" / "GRI.json"
    
    if not json_path.exists():
        logger.error(f"GRI.json 파일을 찾을 수 없습니다: {json_path}")
        raise FileNotFoundError(f"GRI.json not found at {json_path}")
    
    logger.info(f"GRI.json 파일 로드 중: {json_path}")
    ...
    if isinstance(data, dict) and "data_points" in data:
        ...
        logger.error("GRI.json 형식이 올바르지 않습니다")
        raise ValueError("Invalid GRI.json format")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GRI.json 데이터를 data_points 테이블에 로드하고 임베딩 생성"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("GRI.json -> data_points 테이블 로드 스크립트")
    logger.info("=" * 60)
