"""rulebooks.json 데이터를 rulebooks 테이블에 로드하고 임베딩을 생성하는 스크립트
...
사용법:
    python load_rulebooks.py
    python load_rulebooks.py --force  # 기존 데이터 덮어쓰기
    python load_rulebooks.py --dry-run  # 실제 저장 없이 테스트
"""
import sys
import os
import json
...
def load_rulebooks_json() -> List[Dict[str, Any]]:
    """rulebooks.json 파일에서 rulebook 섹션 로드
    ...
    if not json_path.exists():
        logger.error(f"rulebooks.json 파일을 찾을 수 없습니다: {json_path}")
        raise FileNotFoundError(f"rulebooks.json not found at {json_path}")
    
    logger.info(f"rulebooks.json 파일 로드 중: {json_path}")
    ...
    if isinstance(data, dict) and "rulebooks" in data:
        ...
        logger.error("rulebooks.json 형식이 올바르지 않습니다")
        raise ValueError("Invalid rulebooks.json format")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="rulebooks.json 데이터를 rulebooks 테이블에 로드하고 임베딩 생성"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("rulebooks.json -> rulebooks 테이블 로드 스크립트")
    logger.info("=" * 60)
