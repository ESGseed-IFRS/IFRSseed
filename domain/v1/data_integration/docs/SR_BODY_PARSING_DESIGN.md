...
**관련**: 이미지 파이프라인은 [`SR_IMAGES_PARSING_DESIGN.md`](./SR_IMAGES_PARSING_DESIGN.md)를 참고하세요.
...
- ✅ **인덱스 파이프라인**: `SRIndexAgent` + Docling/LlamaParse → `sr_report_index` 저장 완료
...
1. **PDF → 페이지별 텍스트** 추출 (Docling/LlamaParse)
2. **에이전트 자율 실행**: `SRBodyAgent`가 페이지 범위·파싱 전략을 판단하고 도구를 조합
...
- `AGENTIC_INDEX_DESIGN.md` — 인덱스 에이전트 구조 (본문 에이전트 참고용)
...
이 설계대로 구현하면 `sr_report_index`와 동일한 수준의 **에이전트 자율 실행 + 도구 조합** 구조로 본문 파싱·저장이 가능합니다.
