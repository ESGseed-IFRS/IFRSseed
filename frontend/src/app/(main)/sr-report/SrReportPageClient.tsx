...
function parseHoldingTab(param: string | null, hasSectionDeepLink: boolean): HoldingSrTabId {
  if (param === 'h-write' || param === 'h-gen' || param === 'h-aggregate-write') return param;
  if (hasSectionDeepLink) return 'h-write';
...
export function SrReportPageClient() {
  ...
  const hasSectionDeepLink = Boolean(sectionIdParam);
  const holdingTab = parseHoldingTab(holdingTabParam, hasSectionDeepLink);

  const [dashboardWriteKeyword, setDashboardWriteKeyword] = useState<string | null>(null);
  const [dashboardAggregateDpId, setDashboardAggregateDpId] = useState<string | null>(null);
  const [dashboardFocusEntityId, setDashboardFocusEntityId] = useState<string | null>(null);

  useEffect(() => {
    const sid = searchParams.get('sectionId');
    const dpc = searchParams.get('dpCode');
    const eid = searchParams.get('entityId');
    if (sid) setDashboardWriteKeyword(keywordFromDashboardSectionId(sid));
    if (dpc) setDashboardAggregateDpId(dpMasterIdFromMatrixDpCode(dpc));
    if (eid) setDashboardFocusEntityId(eid);
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    if (!changed) return;
    next.set('mode', 'holding');
    next.set('dpId', activeDpId);
    next.set('holdingTab', holdingTab);
    router.replace(`/sr-report?${next.toString()}`);
  }, [workspace, searchParams, router, holdingTab, activeDpId]);
        ...
        <main style={{ flex: 1, minWidth: 0, height: '100%', display: 'flex' }}>
          {workspace === 'holding' ? (
            <HoldingSrWorkspace
              ...
              onTabChange={handleSelectHoldingTab}
              dashboardWriteKeyword={dashboardWriteKeyword}
              onDashboardWriteKeywordConsumed={() => setDashboardWriteKeyword(null)}
              dashboardAggregateDpId={dashboardAggregateDpId}
              dashboardFocusEntityId={dashboardFocusEntityId}
            />
