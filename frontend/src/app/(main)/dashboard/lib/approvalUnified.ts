...
export type ApprovalDomainUnified = 'sr' | 'ghg' | 'audit';
...
const ALL_APPROVAL_MENU_KEYS: ApprovalMenuKey[] = [
  'inbox.request',
  'inbox.history',
  'inbox.cc',
  'outbox.progress',
...
function emptyMenuDomainRecord(): Record<ApprovalDomainUnified, number> {
  return { ghg: 0, sr: 0, audit: 0 };
}

/** 사이드바 배지용 — 메뉴별 총 건수(통합 목과 동일) */
export function countUnifiedDocsByMenu(docs: ApprovalDocUnified[]): Record<ApprovalMenuKey, number> {
  const base = Object.fromEntries(ALL_APPROVAL_MENU_KEYS.map((k) => [k, 0])) as Record<
    ApprovalMenuKey,
    number
  >;
  for (const d of docs) {
...
/** 오버뷰 타일용 — 메뉴 × 도메인 */
export function countUnifiedDocsByMenuDomain(
  docs: ApprovalDocUnified[],
): Record<ApprovalMenuKey, Record<ApprovalDomainUnified, number>> {
  const out = Object.fromEntries(
    ALL_APPROVAL_MENU_KEYS.map((k) => [k, emptyMenuDomainRecord()]),
  ) as Record<ApprovalMenuKey, Record<ApprovalDomainUnified, number>>;
  for (const d of docs) {
    out[d.menuKey][d.domain] += 1;
  }
  return out;
}
...
/** 결재선 편집 시 추가 가능한 인물 풀(mock) */
export const APPROVAL_PEOPLE_POOL: ApprovalPerson[] = [
  { id: 'u1', name: '김환경', dept: '미라콤 ENV팀', title: '매니저' },
  { id: 'u2', name: '이검토', dept: '미라콤 ESG팀' },
  { id: 'u3', name: '박승인', dept: '지주 ESG팀' },
  { id: 'u4', name: '최지주', dept: '지주 ESG팀' },
  { id: 'u5', name: '정데이터', dept: '시큐아이 운영팀' },
  { id: 'u6', name: '한공시', dept: '삼성SDS 지속가능경영팀' },
  { id: 'u7', name: '오산정', dept: '엠로 EHS팀' },
  { id: 'u8', name: '감사담당', dept: '지주 감사팀', title: '팀장' },
  { id: 'me', name: '본인', dept: '지속가능경영팀' },
  { id: 'sys', name: '플랫폼', dept: '시스템' },
];
...
export const APPROVAL_UNIFIED_MOCK: ApprovalDocUnified[] = [
  ...
  },
  {
    ...
    menuKey: 'inbox.request',
    status: 'pending',
    entitySnapshot: snap('datacenter', '판교 DC', '삼성에스디에스(주)', 'DC-PG-01'),
    draftedAt: '2026-03-24T11:00:00+09:00',
    drafter: { id: 'u8', name: '감사담당', dept: '지주 감사팀', title: '팀장' },
    dept: '지주 감사팀',
    retention: '7년',
    title: 'GHG 감사 이벤트(전력 증빙) 확인 요청',
    opinion: '감사 추적용으로 원본 산정 근거와 승인 이력을 확인합니다.',
    bodyHtml: '<p>감사 범위: Scope 2 전력 증빙 교체 건 · 관련 문서 <strong>ESG-2026-0401</strong> 연계</p>',
    attachments: [{ name: 'audit_checklist_v1.pdf', size: '120KB' }],
    approvalLines: [
      { role: '기안', people: [{ id: 'u8', name: '감사담당', dept: '지주 감사팀' }] },
      { role: '검토', people: [{ id: 'u2', name: '이검토', dept: '미라콤 ESG팀' }] },
      { role: '승인', people: [{ id: 'u3', name: '박승인', dept: '지주 ESG팀' }] },
    ],
    links: { ghgAuditEventId: 'ghg-audit-0402', previousDocId: 'ESG-2026-0401' },
    updatedAt: '2026-03-24T11:00:00+09:00',
  },
  {
    id: 'ESG-2026-0398',
    domain: 'sr',
...
/** 결재요청 메뉴 중 대기·진행 건수(상단/알림 배지) */
export function getApprovalInboxBadgeCountForDocs(docs: ApprovalDocUnified[]): number {
  return docs.filter(
...
export function getApprovalInboxBadgeCount(): number {
  return getApprovalInboxBadgeCountForDocs(APPROVAL_UNIFIED_MOCK);
}

export function cloneApprovalUnifiedMock(): ApprovalDocUnified[] {
  return JSON.parse(JSON.stringify(APPROVAL_UNIFIED_MOCK)) as ApprovalDocUnified[];
}
