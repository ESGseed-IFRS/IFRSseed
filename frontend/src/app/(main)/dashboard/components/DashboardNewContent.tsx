...
import {
  ...
  type ApprovalDomain,
  APPROVAL_COUNTS_HOLDING,
  APPROVAL_COUNTS_SUBSIDIARY,
  HOLDING_AFFILIATE_GHG,
  HOLDING_AFFILIATE_SR,
  getHoldingSelfGhg,
...
export function DashboardNewContent({
  ...
  const renderOverview = () => {
    if (mode === 'subsidiary') {
      ...
      const srInboxNeed = countsBase['inbox.request'].sr;
      const srCcNeed = countsBase['inbox.cc'].sr;

      const inboxRequestTotal = countsBase['inbox.request'].ghg + countsBase['inbox.request'].sr;
      const inboxHistoryTotal = countsBase['inbox.history'].ghg + countsBase['inbox.history'].sr;
      const inboxCcTotal = countsBase['inbox.cc'].ghg + countsBase['inbox.cc'].sr;
      ...
      const outboxRejectedTotal = countsBase['outbox.rejected'].ghg + countsBase['outbox.rejected'].sr;
    ...
    // Holding: 기존 구조 유지(전사 관점)
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <AlertBanner
          type="info"
          msg="지주 전체: 본사+계열사 GHG 현황과 SR 취합/검토 흐름을 연결해 확인합니다."
        />

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10 }}>
          <StatTile
            label="Raw data 충실도"
            value={`${ghgHoldSelf.rawCompletenessPct}%`}
            sub="필수 항목 기준"
            accent={C.teal}
          />
          <StatTile
            label="이상치(미조치)"
            value={`${ghgHoldSelf.anomalyOpen}건`}
            sub="규칙 엔진·검증"
            accent={C.amber}
          />
          <StatTile
            label="산정 적합성"
            value={`${ghgHoldSelf.calcFitPct}%`}
            sub="계수·경계·필수 검증"
            accent={C.blue}
          />
          <StatTile
            label="미결 SR 결재"
            value={`${countsBase['inbox.request'].sr}건`}
            sub="수신함 기준"
            accent={C.red}
          />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Card>
            <CTitle>GHG 진행 상태</CTitle>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <WorkflowBadge status={ghgHoldSelf.pipeline} />
              <span style={{ fontSize: 11, color: C.g500 }}>산정 파이프라인 단계</span>
            </div>
            <Pbar pct={ghgHoldSelf.rawCompletenessPct} color={C.teal} />
          </Card>
          <Card>
            <CTitle>SR 공시 DP 패키지</CTitle>
            <div style={{ fontSize: 12, color: C.g600 }}>
              계열사 제출{' '}
              {HOLDING_AFFILIATE_SR.filter((r) => r.submit === 'submitted').length}/
              {HOLDING_AFFILIATE_SR.length} · 승인완료{' '}
              {HOLDING_AFFILIATE_SR.filter((r) => r.review === 'approved').length}건
            </div>
          </Card>
        </div>

        <Card>
          <CTitle>계열사 GHG 요약</CTitle>
