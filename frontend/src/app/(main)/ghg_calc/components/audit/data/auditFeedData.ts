...
/** 계열사: 본인 법인 스코프 이벤트만. 지주: 전체. */
export function filterAuditEventsForSession(
  events: AuditEventDTO[],
  opts: { tenantType: 'holding' | 'subsidiary'; legalEntityId: GhgLegalEntityId | null },
): AuditEventDTO[] {
  if (opts.tenantType === 'holding') return events;
  const id = opts.legalEntityId;
  if (!id) return events;
  return events.filter((e) => e.legalEntityId === id);
}
...
export function buildAuditFeedEvents(approvalMap?: Record<string, ApprovalStep[]>): AuditEventDTO[] {
  ...
  return timeline.map((ev) => {
    ...
    let legalEntityId: GhgLegalEntityId | undefined;
