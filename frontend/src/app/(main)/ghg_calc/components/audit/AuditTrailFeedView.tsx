...
import { buildApprovalInboxFeedEvents, filterAuditEventsForSession } from './data/auditFeedData';
...
function ExpandedPanel({
  ev,
  unified,
  dashboardApprovalHref,
}: {
  ev: AuditEventDTO;
  unified: UnifiedTimelineEvent | undefined;
  dashboardApprovalHref: string;
}) {
...
export function AuditTrailFeedView({
  ...
  const events = useMemo(() => {
    const raw = buildApprovalInboxFeedEvents(approvalMap ?? {});
    return filterAuditEventsForSession(raw, {
      tenantType: session.tenantType,
      legalEntityId: session.legalEntityId,
    });
  }, [approvalMap, session.tenantType, session.legalEntityId]);
        ...
        <div className={`flex flex-wrap items-start justify-between gap-3 ${FEED_CONTAINER}`}>
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              데이터 변경 계보와 결재 이력을 투명하게 추적합니다.
              {session.tenantType === 'subsidiary' && session.corpDisplayName && (
                <span className="mt-1 block text-xs text-slate-400">
                  (계열사 모드: {session.corpDisplayName} 법인 관련 이벤트만 표시)
                </span>
              )}
            </p>
          </div>
            ...
            filtered.map((ev) => {
              ...
              return (
                ...
                >
                  ...
                  {open && (
                    <ExpandedPanel ev={ev} unified={timelineById.get(ev.id)} dashboardApprovalHref={dashboardApprovalHref} />
                  )}
                </div>
              );
            })
