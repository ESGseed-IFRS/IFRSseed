...
import { PERIOD_TYPES, ENERGY_SUB_TYPES, POLLUTANT_SUB_TYPES } from "../../lib/constants";
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-[#f8fafc] border-b-2 border-gray-200">
            <th className="px-3 py-2.5 text-left text-gray-600">시설명</th>
            <th className="px-3 py-2.5 text-left text-gray-600">폐기물 종류</th>
            <th className="px-3 py-2.5 text-left text-gray-600">처리방법</th>
            <th className="px-3 py-2.5 text-left text-gray-600">단위</th>
            <th className="px-3 py-2.5 text-right text-gray-600">1월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">2월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">3월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">합계</th>
            <th className="px-3 py-2.5 text-left text-gray-600">위탁업체</th>
            <th className="px-3 py-2.5 text-center text-gray-600">상태</th>
            <th className="px-3 py-2.5 text-center text-gray-600">관리</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={row.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 1 ? "bg-white" : "bg-[#fafbfc]"}`}>
              <td className="px-3 py-2.5 text-gray-700" style={{ fontWeight: 500 }}>{row.facility}</td>
              <td className="px-3 py-2.5 text-gray-600">{row.wasteType}</td>
              <td className="px-3 py-2.5 text-gray-500">{row.disposalMethod}</td>
              <td className="px-3 py-2.5 text-gray-500">{row.unit}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.jan}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.feb}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.mar}</td>
              <td className="px-3 py-2.5 text-right text-gray-800" style={{ fontWeight: 600 }}>{row.total}</td>
              <td className="px-3 py-2.5 text-gray-500 text-xs">{row.vendor}</td>
              <td className="px-3 py-2.5 text-center"><StatusBadge status={row.status} /></td>
              <td className="px-3 py-2.5 text-center">
                <div className="flex items-center justify-center gap-1">
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-[#f8fafc] border-b-2 border-gray-200">
            <th className="px-3 py-2.5 text-left text-gray-600">시설명</th>
            <th className="px-3 py-2.5 text-left text-gray-600">배출구명</th>
            <th className="px-3 py-2.5 text-left text-gray-600">오염물질</th>
            <th className="px-3 py-2.5 text-left text-gray-600">단위</th>
            <th className="px-3 py-2.5 text-right text-gray-600">1월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">2월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">3월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">평균</th>
            <th className="px-3 py-2.5 text-right text-gray-600">법적기준</th>
            <th className="px-3 py-2.5 text-center text-gray-600">상태</th>
            <th className="px-3 py-2.5 text-center text-gray-600">관리</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={row.id} className={`border-b border-gray-100 hover:bg-gray-50 ${row.status === "exceed" ? "bg-red-50" : row.status === "warning" ? "bg-yellow-50" : i % 2 === 1 ? "bg-white" : "bg-[#fafbfc]"}`}>
              <td className="px-3 py-2.5 text-gray-700" style={{ fontWeight: 500 }}>{row.facility}</td>
              <td className="px-3 py-2.5 text-gray-600">{row.outletName}</td>
              <td className="px-3 py-2.5 text-gray-700">{row.pollutant}</td>
              <td className="px-3 py-2.5 text-gray-500">{row.unit}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.jan}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.feb}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.mar}</td>
              <td className={`px-3 py-2.5 text-right ${row.status === "exceed" ? "text-red-600" : "text-gray-700"}`} style={{ fontWeight: 600 }}>{row.avg}</td>
              <td className="px-3 py-2.5 text-right text-gray-500">{row.legalLimit}</td>
              <td className="px-3 py-2.5 text-center"><StatusBadge status={row.status} /></td>
              <td className="px-3 py-2.5 text-center">
                <div className="flex items-center justify-center gap-1">
                  <button className="p-1 rounded hover:bg-blue-100 text-gray-400 hover:text-blue-600"><Edit2 size={12} /></button>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-[#f8fafc] border-b-2 border-gray-200">
            <th className="px-3 py-2.5 text-left text-gray-600">시설명</th>
            <th className="px-3 py-2.5 text-left text-gray-600">약품명</th>
            <th className="px-3 py-2.5 text-left text-gray-600">CAS No.</th>
            <th className="px-3 py-2.5 text-left text-gray-600">단위</th>
            <th className="px-3 py-2.5 text-right text-gray-600">1월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">2월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">3월</th>
            <th className="px-3 py-2.5 text-right text-gray-600">합계</th>
            <th className="px-3 py-2.5 text-center text-gray-600">유해물질 분류</th>
            <th className="px-3 py-2.5 text-center text-gray-600">상태</th>
            <th className="px-3 py-2.5 text-center text-gray-600">관리</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={row.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 1 ? "bg-white" : "bg-[#fafbfc]"}`}>
              <td className="px-3 py-2.5 text-gray-700" style={{ fontWeight: 500 }}>{row.facility}</td>
              <td className="px-3 py-2.5 text-gray-700">{row.chemicalName}</td>
              <td className="px-3 py-2.5 text-gray-500 font-mono">{row.casNo}</td>
              <td className="px-3 py-2.5 text-gray-500">{row.unit}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.jan}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.feb}</td>
              <td className="px-3 py-2.5 text-right text-gray-700">{row.mar}</td>
...
export function RawDataUpload({ category }: RawDataUploadProps) {
  ...
  const getAlertCount = () => {
    ...
    if (category === "waste") return wasteData.filter(d => d.status === "error").length;
    return 0;
  };
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400">
              총 {category === "energy" ? filteredEnergyData.length : category === "pollution" ? filteredPollutionData.length : category === "waste" ? wasteData.length : category === "chemical" ? chemicalData.length : category === "energy-provider" ? energyProviderData.length : consignmentData.length}건
            </span>
            {category === "energy" && (
        ...
        <div className="overflow-hidden">
          ...
          {category === "waste" && <WasteTable data={wasteData} />}
          {category === "pollution" && <PollutionTable data={filteredPollutionData} />}
          {category === "chemical" && <ChemicalTable data={chemicalData} />}
          {category === "energy-provider" && <EnergyProviderTable />}
          {category === "consignment" && <ConsignmentTable />}
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <span>1 / 1 페이지</span>
            <span>·</span>
            <span>{selectedYear}년 · {periodType}</span>
            <span>·</span>
