...
const anomalies: Anomaly[] = [
  { id: 1, facility: "생산동A", scope: "Scope 1", dataType: "LNG 사용량", period: "2026-02", currentValue: 55800, prevValue: 12450, changeRate: 348.2, unit: "Nm³", status: "unresolved" },
  { id: 2, facility: "생산동B", scope: "Scope 2", dataType: "전력 사용량", period: "2026-01", currentValue: 654780, prevValue: 1240000, changeRate: -47.2, unit: "kWh", status: "reason_added", reason: "2025년 동기 설비 증설로 인한 일시적 증가분 제거, 정상 운영 복귀" },
  { id: 3, facility: "본관동", scope: "Scope 1", dataType: "냉매 사용량 (HFC)", period: "2026-02", currentValue: 5.95, prevValue: 0.0, changeRate: 999, unit: "kg", status: "corrected", reason: "냉방 시스템 교체 후 초기 냉매 충전 작업 (일회성)", correctedValue: 5.95 },
  { id: 4, facility: "유틸리티동", scope: "Scope 2", dataType: "용수 사용량", period: "2026-03", currentValue: 8750, prevValue: 18200, changeRate: -51.9, unit: "m³", status: "unresolved" },
  { id: 5, facility: "연구동", scope: "Scope 3", dataType: "폐기물 배출량", period: "2026-01", currentValue: 850, prevValue: 92, changeRate: 823.9, unit: "kg", status: "ignored", reason: "연구동 리모델링으로 인한 일회성 폐기물 증가 (확인 완료)" },
];
    ...
    <div className="p-5 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">전년비 ±50% 초과 등 자동 감지된 이상치 항목에 대해 사유 입력 또는 보정 처리</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 relative">
          ...
          <input placeholder="시설명 검색..." className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-1.5 text-xs focus:outline-none focus:border-blue-400" />
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {filtered.map((anomaly) => {
            ...
            const isHighAlert = Math.abs(anomaly.changeRate) >= 100;
            return (
              <div key={anomaly.id} className={isHighAlert && anomaly.status === "unresolved" ? "bg-red-50/30" : ""}>
                <div
                ...
                >
                  ...
                  <div className="text-right shrink-0 w-36">
                    <div className="text-xs text-gray-500">전년 동월</div>
                    <div className="text-xs text-gray-600">{anomaly.prevValue.toLocaleString()} {anomaly.unit}</div>
                  </div>
                  <div className={`text-right shrink-0 w-24 text-xs ${Math.abs(anomaly.changeRate) >= 100 ? "text-red-600" : "text-orange-500"}`} style={{ fontWeight: 700 }}>
                    {anomaly.changeRate > 0 ? "+" : ""}{anomaly.changeRate > 999 ? ">999" : anomaly.changeRate.toFixed(1)}%
                  </div>
                      ...
                      {anomaly.status === "unresolved" && (
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-emerald-700 bg-emerald-50 border border-emerald-300 rounded-lg hover:bg-emerald-100 transition-colors">
                              보정값 입력
                            </button>
                            <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-500 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
