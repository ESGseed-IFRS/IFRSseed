...
import { GHG_SUBSIDIARY_ENTITY_ROWS } from '../../lib/groupEmissionEntities';
import {
  GHG_SCOPE_MONTH_KEYS,
  GHG_SCOPE_MONTH_LABELS,
  buildScopeCalculationDataset,
...
export function ScopeCalculation() {
  ...
  const dataset = useMemo(() => {
    if (session.tenantType === 'holding') {
      return buildScopeCalculationDataset({ mode: 'holding', subsidiaryRow: null });
    }
    const r = session.legalEntityId
      ? GHG_SUBSIDIARY_ENTITY_ROWS.find((x) => x.legalEntityId === session.legalEntityId)
      : undefined;
    return buildScopeCalculationDataset({ mode: 'subsidiary', subsidiaryRow: r ?? null });
  }, [session.tenantType, session.legalEntityId]);
    ...
    return (
      <div className="p-5">
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-sm text-amber-900">
          Scope 산정 데이터가 없습니다. 계열사 세션과 법인 키(
          <code className="text-xs">NEXT_PUBLIC_GHG_LEGAL_ENTITY</code>)를 확인해 주세요.
        </div>
      </div>
    );
  }

  const { totals, grandTotal, prev, monthlyChart, scope1Categories, scope2Categories, scope3Categories, label } =
    dataset;
  ...
  return (
    <div className="p-5 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">
            입력 데이터와 배출계수 기반 Scope 1·2·3 산정 — 산정 결과 조회·GHG 보고서와 동일 정본(
            <code className="text-[10px]">groupEmissionEntities</code> + 월별 파생)
          </p>
          <p className="text-blue-700/80 text-[11px] mt-1">
            기준: <span style={{ fontWeight: 600 }}>{label}</span>
            {session.tenantType === 'holding' ? ' · 항목 표는 그룹 연간 Scope에 맞춰 비율 스케일된 데모 라인입니다.' : ''}
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="p-4">
          {activeScope === 'scope1' && <CategoryTable categories={scope1Categories} />}
          {activeScope === 'scope2' && <CategoryTable categories={scope2Categories} />}
          {activeScope === 'scope3' && <CategoryTable categories={scope3Categories} />}
        </div>
