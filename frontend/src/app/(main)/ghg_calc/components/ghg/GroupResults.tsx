...
import {
  GHG_SUBSIDIARY_ENTITY_ROWS as subsidiaryRows,
  GHG_DOMESTIC_SITE_ROWS as domesticSiteRows,
  GHG_ALL_GROUP_ENTITIES as allEntities,
  type GroupEmissionEntityRow as GroupEntityRow,
} from "../../lib/groupEmissionEntities";
...
function SubsidiaryEmissionResults() {
  ...
  const id = session.legalEntityId;
  const row = id ? subsidiaryRows.find((r) => r.legalEntityId === id) : undefined;
    ...
    return (
      <div className="p-5">
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-sm text-amber-900">
          소속 법인({id ?? '미지정'})에 대한 산정 요약 데이터가 없습니다. 세션의 법인 키를 확인해 주세요.
        </div>
      </div>
    );
  }

  const chg = ((row.total - row.prev) / row.prev) * 100;
  const changeRate = chg.toFixed(1);
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-gray-900">산정 결과 조회 — {row.name}</h1>
            <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full border border-blue-200">계열사</span>
          </div>
          <p className="text-gray-500 text-xs mt-0.5">귀사 법인 기준 GHG 산정 결과만 표시됩니다. 그룹 통합 집계는 지주 사용자에게만 제공됩니다.</p>
        </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            <tr className="bg-[#fafbfc] border-b border-gray-100">
              ...
              <td className="px-4 py-2.5 text-right text-gray-600">{row.scope2.toLocaleString()}</td>
              ...
              <td className="px-4 py-2.5 text-center">
                {row.frozen ? <Snowflake size={14} className="inline text-blue-400" /> : <span className="text-yellow-500 text-xs">진행중</span>}
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="bg-white border border-orange-100 rounded-xl p-4">
          ...
          <div className="text-gray-900 tabular-nums break-all" style={{ fontSize: '18px', fontWeight: 800 }}>
            {formatTco2eq(displayed.reduce((s, c) => s + c.scope2, 0))}
          </div>
          <div className="text-gray-400 text-xs">tCO₂eq</div>
        </div>
        <div className="bg-white border border-purple-100 rounded-xl p-4">
