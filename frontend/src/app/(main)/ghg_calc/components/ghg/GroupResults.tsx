...
const corporations = [
  { name: "미라콤", scope1: 2180, scope2: 9540, scope3: 18240, total: 29960, prev: 28450, frozen: true },
  { name: "시큐아이", scope1: 5420, scope2: 18230, scope3: 32100, total: 55750, prev: 54210, frozen: true },
  { name: "에스코어", scope1: 840, scope2: 3210, scope3: 6780, total: 10830, prev: 11240, frozen: true },
  { name: "멀티캠퍼스", scope1: 3290, scope2: 4180, scope3: 12450, total: 19920, prev: 19100, frozen: true },
  { name: "엠로", scope1: 6780, scope2: 22100, scope3: 41200, total: 70080, prev: 71500, frozen: false },
  { name: "오픈핸즈", scope1: 1230, scope2: 5670, scope3: 9870, total: 16770, prev: 16320, frozen: true },
];
...
export function GroupResults() {
  ...
  const displayed = showFrozenOnly ? corporations.filter(c => c.frozen) : corporations;
    ...
    <div className="p-5 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">전체 계열사 GHG 배출량 통합 집계 및 추세 파악</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="col-span-2 bg-white border border-gray-200 rounded-xl p-4">
          <h3 className="text-gray-800 mb-3" style={{ fontSize: "13px" }}>법인별 동결 현황</h3>
          <div className="space-y-2">
            {corporations.map((corp) => (
              <div key={corp.name} className="flex items-center justify-between py-1.5 border-b border-gray-100 last:border-0">
                <div className="flex items-center gap-2">
                  {corp.frozen ? <Snowflake size={12} className="text-blue-400" /> : <div className="w-3 h-3 rounded-full border-2 border-gray-300"></div>}
                  <span className="text-xs text-gray-700">{corp.name}</span>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full border ${corp.frozen ? "text-blue-600 bg-blue-50 border-blue-200" : "text-yellow-600 bg-yellow-50 border-yellow-200"}`}>
          ...
          <div className="mt-3 pt-2 border-t border-gray-100 text-xs text-gray-400 text-center">
            {corporations.filter(c => c.frozen).length} / {corporations.length} 법인 동결 완료
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          <h3 className="text-gray-800" style={{ fontSize: "13px" }}>법인별 배출량 비교 테이블</h3>
        </div>
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-[#f8fafc] border-b border-gray-200">
              <th className="px-4 py-2.5 text-left text-gray-500">법인명</th>
          ...
          <tbody>
            {displayed.map((corp, i) => {
              ...
              return (
                <tr key={corp.name} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 1 ? "bg-white" : "bg-[#fafbfc]"}`}>
            ...
            <tr className="bg-[#0d1b36] text-white">
              <td className="px-4 py-3 text-xs" style={{ fontWeight: 700 }}>그룹 합계</td>
              ...
              <td className="px-4 py-3 text-center text-xs text-white/60">
                {corporations.filter(c => c.frozen).length}/{corporations.length}
              </td>
            </tr>
          </tbody>
        </table>
