...
/** 통합 감사 단일 화면 + 결재함만 유지 (4탭 ID 제거) */
export type AuditSubTab = 'unified' | 'approval';
...
interface SidebarProps {
  ...
  setAuditSubTab: (t: AuditSubTab) => void;
  rawDataCategory: RawDataCategory;
  setRawDataCategory: (c: RawDataCategory) => void;
}

const calcOtherChildren: { id: CalcSubTab; label: string; icon: React.ReactNode; badge?: string }[] = [
...
/** UNIFIED=한 타임라인 조회 / WITH_APPROVAL=승인·반려 액션 — AUDIT_TRAIL_SCREEN_STORY.md 참고 */
const auditChildren: { id: AuditSubTab; label: string; icon: React.ReactNode }[] = [
  { id: 'unified', label: '통합 감사 추적', icon: <Shield size={13} /> },
  { id: 'approval', label: '결재함', icon: <Inbox size={13} /> },
];
...
export function Sidebar({ mainTab, setMainTab, calcSubTab, setCalcSubTab, auditSubTab, setAuditSubTab, rawDataCategory, setRawDataCategory }: SidebarProps) {
          ...
          {openMenus.includes("audit-section") && (
            <div className="pb-1">
              {auditChildren.map((child) => {
                const active = isAuditActive && auditSubTab === child.id;
                return (
                  <button
                    key={child.id}
                    type="button"
